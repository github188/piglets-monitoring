...
// 阵列错误码
#define NET_DVR_NAME_NOT_ONLY               (RAID_ERROR_INDEX + 0)  // 用户名不唯一
#define NET_DVR_OVER_MAX_ARRAY              (RAID_ERROR_INDEX + 1 ) // 阵列达到上限
#define NET_DVR_OVER_MAX_VD                 (RAID_ERROR_INDEX + 2 ) // 虚拟磁盘达到上限
#define NET_DVR_VD_SLOT_EXCEED              (RAID_ERROR_INDEX + 3 ) // 虚拟硬盘槽位已满
#define NET_DVR_PD_STATUS_INVALID           (RAID_ERROR_INDEX + 4 ) // 重建阵列所需物理硬盘状态错误
#define NET_DVR_PD_BE_DEDICATE_SPARE        (RAID_ERROR_INDEX + 5 ) // 重建阵列所需物理硬盘为指定热备
#define NET_DVR_PD_NOT_FREE                 (RAID_ERROR_INDEX + 6 ) // 重建阵列所需物理硬盘非空闲
#define NET_DVR_CANNOT_MIG2NEWMODE          (RAID_ERROR_INDEX + 7 ) // 不能从当前的raid类型迁移到新的RAid类型
#define NET_DVR_MIG_PAUSE                   (RAID_ERROR_INDEX + 8 ) // 迁移操作已暂停
#define NET_DVR_MIG_ABOUTED                 (RAID_ERROR_INDEX + 9 ) // 迁移操作已取消
...
#define NET_DVR_LD_IS_FUNCTIONAL            (RAID_ERROR_INDEX + 14) // Specified virtual disk is functional正常工作，不能重建
...
{   
    ...
    BYTE    byRes1[3];  // 保留字节
    BYTE    byRuleName[NAME_LEN];   // 规则名称 
    DWORD   dwEvnetType;    // 交通事件检测类型 TRAFFIC_AID_TYPE
    NET_VCA_SIZE_FILTER struSizeFilter; // 尺寸过滤器
...
typedef struct tagNET_DVR_IO_OUTCFG
{
	...
	BYTE    byFreqMulti;		//倍频，数值范围[1,10]
	BYTE    byDutyRate;		//占空比，10% 15% 20% 25% 30%
    BYTE    byRes2[2];
...
typedef struct tagNET_DVR_PLCCFG
{
	...
	BYTE byBrightChangeEnable;	//是否考虑光照变化（默认启用）：0-关闭，1-启用 
	BYTE byBrightChangeThreld;  //判断光照变化的阈值（默认值50），范围[0, 100]
...
typedef struct
{
	...
	BYTE byEsataUseage;		//Esata的默认用途，0-默认录像，1-默认备份
...
NET_DVR_API BOOL __stdcall NET_DVR_CalcArraySize(LONG lUserID, LPNET_DVR_OPERATE_ARRAY_PARAM lpArrayParam, unsigned __int64* lpArraySize);
